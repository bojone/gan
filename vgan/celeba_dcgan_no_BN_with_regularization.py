...
""" initialization """
ckpt_dir = './checkpoints/celeba_dcgan_no_bn_regular'
...
try:
    ...
    for it in range(sess.run(it_cnt), max_it):
        ...
        # train G
        z_ipt = np.random.normal(size=[batch_size * repeat, z_dim])
        old_fake = sess.run(fake, feed_dict={z: z_ipt})
        z_ipt = z_ipt.reshape((repeat, batch_size, z_dim))
        old_fake = old_fake.reshape((repeat, batch_size) + old_fake.shape[1:])
        
        for i in range(repeat):
            # train G
            sess.run([g_step], feed_dict={z: z_ipt[i], real: old_fake[i]})
            g_summary_opt, _ = sess.run([g_summary, g_step], feed_dict={z: z_ipt[i], real: old_fake[i]})
